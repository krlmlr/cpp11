...
cpp_register <- function(
  ...
  init <- generate_init_functions(get_registered_functions(
    all_decorations,
    "cpp11::init",
    quiet
  ))
  ...
  cpp_function_registration <- glue::glue_data(
    funs,
    '    {{
  ...
  cpp_function_registration <- glue::glue_collapse(
    cpp_function_registration,
    sep = "\n"
  )
...
get_registered_functions <- function(
  decorations,
  tag,
  quiet = !is_interactive()
) {
  ...
  out$functions <- lapply(
    out$context,
    decor::parse_cpp_function,
    is_attribute = TRUE
  )
...
generate_init_functions <- function(funs) {
  ...
  funs <- funs[c("name", "return_type", "args", "file", "line", "decoration")]
  funs$declaration_params <- vcapply(
    funs$args,
    glue_collapse_data,
    "{type} {name}"
  )
...
generate_r_functions <- function(funs, package = "cpp11", use_package = FALSE) {
  ...
  funs$params <- vcapply(funs$list_params, function(x) {
    if (nzchar(x)) paste0(", ", x) else x
  })
  ...
  funs$calls <- ifelse(
    ...
    glue::glue_data(
      funs,
      'invisible(.Call({package_names}{params}{package_call}))'
    ),
...
wrap_call <- function(name, return_type, args) {
  call <- glue::glue(
    '{name}({list_params})',
    list_params = glue_collapse_data(
      args,
      "cpp11::as_cpp<cpp11::decay_t<{type}>>({name})"
    )
  )
  if (return_type == "void") {
...
get_call_entries <- function(path, names, package) {
  ...
  redundant <- glue::glue_collapse(
    glue::glue('extern SEXP _{package}_{names}'),
    sep = '|'
  )
...
get_cpp_register_needs <- function() {
  res <- read.dcf(system.file("DESCRIPTION", package = "cpp11"))[,
...
check_valid_attributes <- function(decorations, file = decorations$file) {
  bad_decor <- startsWith(decorations$decoration, "cpp11::") &
    (!decorations$decoration %in%
      c("cpp11::register", "cpp11::init", "cpp11::linking_to"))
  ...
  if (any(bad_decor)) {
    ...
    msg <- glue::glue(
      "cpp11 attributes must be one of `cpp11::register`, `cpp11::init` or `cpp11::linking_to`:
      {bad_lines}
      "
    )
    stop(msg, call. = FALSE)
